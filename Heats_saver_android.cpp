...
Java_com_example_app_MainActivity_writeFile(JNIEnv* env, jobject /* this , jstring filePath) {
...
vector<vector<float>> create_circogeometry(int x_size, int y_size, int fin_height, int base_thickness, int spacing) {
    vector<vector<float>> img_array(x_size, vector<float>(y_size, base_thickness));
...
file<<to_string(t)<<'\t'<<to_string(sheat)<<endl;
